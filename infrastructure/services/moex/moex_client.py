...
class BaseMoexClient:
        ...
        for attempt in range(1, self.RETRY_COUNT + 1):
            try:
                ...
                if attempt == self.RETRY_COUNT:
                    print(f"Ошибка запроса к {url}: {e}, исчерпаны попытки")
                    return {}
                else:
                    print(f"Ошибка запроса к {url}: {e}, повтор через {self.RETRY_DELAY}с (попытка {attempt})")
    ...
    async def get_candles(self, session: aiohttp.ClientSession, ticker: str, interval: TimeFrame, from_date: Optional[str], till_date: Optional[str]) -> pd.DataFrame:
